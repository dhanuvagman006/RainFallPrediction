...
for seq_len in SEQUENCE_LENGTHS:
    ...
    model = Sequential([
        GRU(64, activation='relu', input_shape=(seq_len, X_train.shape[2]), return_sequences=False),
        Dropout(0.2),
    ...
    print(f"Starting GRU Training ({seq_len} day window)...")
    history = model.fit(X_train, y_train, epochs=25, batch_size=32, validation_split=0.1, verbose=1)

    history_path = history_dir / f"gru_{seq_len}d.json"
    ...
    print("\n" + "=" * 40)
    print(f"       GRU PERFORMANCE METRICS ({seq_len}d)")
    print("=" * 40)
    ...
    model_path = models_dir / f"gru_{seq_len}d.keras"
    model.save(model_path)

    metrics_payload[f"gru_{seq_len}d"] = {
        "nse": float(nse),
        "r2": float(r2),
        "mae": float(mae),
        "rmse": float(rmse),
