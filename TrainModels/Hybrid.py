...
# We use the past 7 days of weather data to predict the current day's rainfall
...
# 4. Build TCNN-LSTM Hybrid Model
model = Sequential([
    # TCNN Part: Extracting local temporal features
    Conv1D(filters=64, kernel_size=3, activation='relu', input_shape=(window_size, len(feature_cols))),
    MaxPooling1D(pool_size=2),
    Dropout(0.2),
    
    # LSTM Part: Learning long-term dependencies
...
# 5. Train the Model
print("Training TCNN-LSTM Hybrid Model...")
...
history_path = history_dir / "hybrid.json"
...
# 6b. Save model + metrics for frontend consumption
models_dir = Path(__file__).resolve().parent / "saved_models"
models_dir.mkdir(parents=True, exist_ok=True)
model_path = models_dir / "hybrid.keras"
