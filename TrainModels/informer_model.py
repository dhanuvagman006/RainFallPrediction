...
# 2. Windowing (Transformers need sequences)
# We look back at the previous 14 days to predict today's rain
SEQUENCE_LENGTH = 14 
...
history_path = history_dir / "transformer.json"
...
model_path = models_dir / "transformer.keras"
...
# 6. Single Prediction Function
def predict_current_rain(last_14_days_features):
    """
    Pass a (14, 11) shape array of the last 14 days of features.
    """
    scaled_input = f_scaler.transform(last_14_days_features)
    input_reshaped = scaled_input.reshape(1, SEQUENCE_LENGTH, len(feature_cols))
    pred_scaled = model.predict(input_reshaped, verbose=0)
    return max(0, t_scaler.inverse_transform(pred_scaled)[0][0])