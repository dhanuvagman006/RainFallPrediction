...
for seq_len in SEQUENCE_LENGTHS:
    ...
    kernel_size = 2 if seq_len >= 2 else 1
    model = Sequential([
        TCN(
    ...
    print(f"Training TCN Model ({seq_len} day window)...")
    history = model.fit(X_train, y_train, epochs=30, batch_size=32, validation_split=0.1, verbose=1)

    history_path = history_dir / f"tcn_{seq_len}d.json"
    ...
    print("\n" + "=" * 40)
    print(f"       TCN PERFORMANCE METRICS ({seq_len}d)")
    print("=" * 40)
    ...
    model_path = models_dir / f"tcn_{seq_len}d.keras"
    model.save(model_path)

    metrics_payload[f"tcn_{seq_len}d"] = {
        "nse": float(nse),
        "r2": float(r2),
        "mae": float(mae),
        "rmse": float(rmse),
